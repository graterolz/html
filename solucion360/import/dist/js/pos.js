/*
 * 20160721 - INI : Gestion mobile
 * Las siguientes lineas de codigo son utilizados para estraer parametros 
 * pasados como queryString en el archivo js. 
 */ 
...
// Con esta variable se determina si el textbox (#add_item) debe tomar el foco
// En las versiones mobiles, no debe tomar el foco pues eso hace que levante el teclado virtual
// en las versiones de PC, debe tomar el foco pues a partir de esto permite al usuario hacer una nueva busqueda
var NavegatorIsMobile = (params['isMobile'] == '1') ? true : false;
...
function loadItems() {
    ...
    if (get('spositems')) {
		...
		// Se asigna el foco del control solo si NO es mobile
        if (NavegatorIsMobile != true) $('#add_item').focus();
    }
}
        ...
        if (count <= 1) {
            ...
            if(total_paying = get('amount')) {
                ...
                if(Settings.rounding != 0) {
                    ...
                    $('#balance').text(formatMoney(total_paying - total));
                    $('#balance_val').val(formatDecimal(total_paying - total));
                    total_paid = total_paying;
                    grand_total = total;
                }
            }
        ...
        $.ajax({
            ...
            success: function(res) {
                if(res.status == 'success') {
                    $('#customer').append($("<option></option>").attr("value",res.id).text(res.val)); 
                    $('#customer').select2('val', res.id);
